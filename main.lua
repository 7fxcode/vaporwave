...
local Mainn = Window:NewTab("Welcome!")

local mainsection = Mainn:NewSection("hi fucktard")

mainsection:NewDropdown("credits", "lmfao", {"ui: kavo", "scripting: 7fx#2922", "total bans: 0"})

local Humandd = Window:NewTab("Local Plr")
...
humanoidstuff:NewSlider("change fov", "if you dk whats it try it", 120, 70, function(s)

    game.Workspace.CurrentCamera.FieldOfView = s

end)

humanoidstuff:NewLabel("reseting")
...
humanoidstuff:NewButton("reset fov", "if you dk whats it try it", function(s)
...
targetting:NewToggle("spectate", "spectate the target", function(state)

    if state then
        getgenv().spectate = true
        while spectate == true do
            game:GetService("Workspace").CurrentCamera.CameraSubject = game:GetService("Players"):FindFirstChild(Target[1]).Character
            wait()
        end
    else
